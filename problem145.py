...
# I cheated :X
# Original brute-force program took ~0.9 second for 10**6, 9 seconds for 10**7
# Would take 15-20 minutes for 10**9
...
limit = 10**7
# working with array lookup so we don't doubly calculate 409 and 904 seems to not be worth it
# based on test with limit=10**6 and limit=10**7

for n in range(limit):
    if n % 10 != 0: # last digit can't be 0
        reversible = True
        rev = str(n)[::-1]
        x = n + int(rev)
        for digit in str(x):
...
print(f'It took {time.time()-start:.3f} seconds to find all {reversible_numbers} reversible numbers below {limit}')
'''
